                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <form action="{{ route('adm.view.order', ['id'=>$order->id]) }}" method="POST">
                                                                        @csrf
                                                                        @method('POST')
                                                                        <button type="submit" class="btn btn-sm btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#gridSystemModalView1{{ $order->id }}">
                                                                            View Details
                                                                        </button>
                                                                    </form>

                                                                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#gridSystemModalUpdate{{ $order->id }}">
                                                                        Update Status
                                                                    </button>

                                                                    <div id="gridSystemModalUpdate{{ $order->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="gridModalLabel{{ $order->id }}" aria-hidden="true">
                                                                        <div class="modal-dialog" role="document">
                                                                            <div class="modal-content">
                                                                                <div class="modal-header">
                                                                                    @if ($order->user)
                                                                                        <h5 class="modal-title" id="gridModalLabel{{ $order->id }}">{{ $order->user->username }}</h5>
                                                                                    @else
                                                                                        <h5 class="modal-title" id="gridModalLabel{{ $order->id }}">User not Found</h5>
                                                                                    @endif
                                                                                        ...
                                                                                        <div class="row">
                                                                                            <div class="col-md-12 col-xs-12">
                                                                                                <div class="form-group">
                                                                                                    ...
                                                                                                    <select id="orderStatus{{ $order->id }}" name="status" class="form-control form-select" required>
                                                                                                    ...
                                                                                                    <label for="estimateDate{{ $order->id }}" id="estimateLabel{{ $order->id }}" class="form-label mt-2">Estimate Delivery Date</label>
                                                                                                    <input type="date" id="estimateDate{{ $order->id }}" name="estimate_date" class="form-control mt-2" required>

                                                                                                    <!-- Reason for Cancellation (Hidden Initially) -->
                                                                                                    <label for="cancelReason{{ $order->id }}" id="reasonLabel{{ $order->id }}" class="form-label mt-2" style="display:none;">Reason for Cancellation</label>
                                                                                                    <textarea id="cancelReason{{ $order->id }}" name="cancel_reason" class="form-control mt-2" style="display:none;"></textarea>
                                                                                                </div>
                                                                                            </div>
                                                                                ...
                                                                                <script>
                                                                                    $(document).ready(function() {
                                                                                        $('#orderStatus{{ $order->id }}').change(function() {
                                                                                            ...
                                                                                            if (status === 'cancelled') {
                                                                                                // Hide Estimate Date and Show Cancel Reason
                                                                                                $('#estimateDate{{ $order->id }}').hide();
                                                                                                $('#estimateLabel{{ $order->id }}').hide();
                                                                                                $('#cancelReason{{ $order->id }}').show();
                                                                                                $('#reasonLabel{{ $order->id }}').show();

                                                                                                // Set required attributes
                                                                                                $('#cancelReason{{ $order->id }}').attr('required', true);
                                                                                                $('#estimateDate{{ $order->id }}').removeAttr('required');
                                                                                            } else {
                                                                                                // Show Estimate Date and Hide Cancel Reason
                                                                                                $('#estimateDate{{ $order->id }}').show();
                                                                                                $('#estimateLabel{{ $order->id }}').show();
                                                                                                $('#cancelReason{{ $order->id }}').hide();
                                                                                                $('#reasonLabel{{ $order->id }}').hide();

                                                                                                // Set required attributes
                                                                                                $('#estimateDate{{ $order->id }}').attr('required', true);
                                                                                                $('#cancelReason{{ $order->id }}').removeAttr('required');
                                                                                            }
                                                                                        });
                                                                                    });
                                                                                </script>
